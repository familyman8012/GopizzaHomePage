...
export const fetchInstaFeed = async () => {
  const data = await AxiosUtil.get(
    `https://graph.instagram.com/me/media?fields=id,caption,media_url,timestamp,media_type,permalink&limit=3&access_token=${process.env.INSTAGRAM_KEY}`
  );
...
//캠페인 - 부울경 프로모션
export const fetchBwk = async (params: IBwk) => {
  const response = await AxiosUtil.post(`/sys/v1/promotion/customer`, params);
