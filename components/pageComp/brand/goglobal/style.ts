...
const countryImg = Array.from({ length: 5 }).map(
  (_, i) =>
    css`
      &:nth-of-type(${i + 1}) {
        background: url("/images/brand/goglobal/img_global${i + 1}x2.webp") no-repeat left top / 100%;
        .country {
          background: url("/images/brand/goglobal/ico_global${i + 1}.webp") no-repeat left top / 100%;
        }
      }
    `
);
